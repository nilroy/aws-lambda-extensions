...
TIMEOUT_MS = 1000
# Maximum time (in milliseconds) that a batch is buffered.
MAX_BYTES = 256 * 1024
# Maximum size in bytes that the logs are buffered in memory.
MAX_ITEMS = 10000
# Maximum number of events that are buffered in memory.
...
def subscibe_listener(extension_id, listener_url):
    ...
    try:
        subscription_request_body = {
            ...
            "buffering": {"timeoutMs": TIMEOUT_MS, "maxBytes": MAX_BYTES, "maxItems": MAX_ITEMS},
        }
        ...
        response = requests.put(
            TELEMETRY_API_URL, data=json.dumps(subscription_request_body), headers=subscription_request_headers
        )
