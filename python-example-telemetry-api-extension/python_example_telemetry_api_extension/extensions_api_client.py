...
def register_extension(extension_name):
    ...
    try:
        registration_request_body = {"events": ["INVOKE", "SHUTDOWN"]}
...
def next(extension_id):
    try:
        ...
        response = requests.get("{0}/event/next".format(REGISTRATION_REQUEST_BASE_URL), headers=next_event_request_header)
