    ...
    def run_forever(self):
        ...
        while True:
            ...
            while not self.queue.empty():
                ...
                s3_filename = (
                    (os.environ["AWS_LAMBDA_FUNCTION_NAME"]) + "-" + (datetime.now().strftime("%Y-%m-%d-%H:%M:%S.%f")) + ".log"
                )
                try:
                    response = s3.Bucket(s3_bucket).put_object(Key=s3_filename, Body=json.dumps(batch))
